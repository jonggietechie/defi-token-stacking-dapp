...
async function loadInitialData(sClass) {
  ...
  try {
    ...
    //CLASS ELEMENT DATA
    let totalLockedTokens = await cObj.methods.getTotalStackedTokens().call();
    ...
    document
      .querySelectorAll(".Minimum-Stacking-Amount")
      .forEach(function (element) {
...
async function unstackTokens() {
  try {
    let nTokens = document.getElementById("amount-to-unstack-value-new").value;
    if (!nTokens) {
      return;
    }
    if (isNaN(nTokens) || nTokens == 0 || Number(nTokens) < 0) {
    ...
    if (balMainUser < nTokens) {
      notyf.error(
        `Insufficient tokens staked on ${SELECT_CONTRACT[_NETWORK_ID].network_name}!`
      );
      return;
    }
    unstackTokensMain(tokenToTransfer, oContractStacking, sClass);
  } catch (error) {
    console.log(error);
    notyf.dismiss(notification);
    notyf.error(formatEthErrorMsg(error));
...
async function claimTokens() {
  try {
    ...
    if (!rewardBal) {
      notyf.dismiss(notification);
      notyf.error(`insufficient rewards to claim!`);
      return;
    }
...
async function claimTokenMain(oContractStacking, sClass) {
  ...
  try {
    gasEstimation = await oContractStacking.methods.claimRewards().estimateGas({
      from: currentAddress,
    });
    console.log("gasEsimation: ", gasEstimation);
  } catch (error) {
    console.log(error);
    notyf.error(formatEthErrorMsg(error));
    return;
